...
0. **確保相依套件就緒（第一次跑才需要，之後可略）**：由你（agent）在對話裡自己跑，別叫使用者開 CMD。
   ...
   - Python：一律 `pip install -U yt-dlp`；轉錄若走本機 GPU 再 `pip install -U faster-whisper`＋**CUDA 執行期 `pip install -U nvidia-cudnn-cu12 nvidia-cublas-cu12`**（faster-whisper/CTranslate2 載入 CUDA 時需要 cuDNN9＋cuBLAS，光有顯卡驅動不夠，缺了會 `Unable to load libcudnn...`）；若走 `--api` 則 `pip install -U openai`（Mac/Linux 用 `pip3` 或 `python3 -m pip`；也可直接 `pip install -r requirements.txt`）。
...
### 4. 轉錄（Transcribe）
`python transcribe.py <audio_dir> <transcript_dir> [--api]`（**Mac/Linux 用 `python3`**）：**自動偵測 GPU**——有 NVIDIA GPU → 本機 faster-whisper large-v3（免費、音檔不離開電腦，需先裝 cuDNN＋cuBLAS 執行期，見步驟 0）；無 GPU → 提示使用者改用 `--api`（需自備 `OPENAI_API_KEY`），並告知每分鐘成本。**提醒使用者：`--api` 模式會把音檔上傳給 OpenAI 轉錄**（作者端收不到，但音檔確實會離開本機到 OpenAI）。
