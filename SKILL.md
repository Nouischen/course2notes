...
**寫完後的強制自我稽核（不做這步＝沒做完）**：回頭掃全文，抓出這些字眼——「圖／圈／圈圈／左邊／右邊／左圖／右下／上圖／下圖／如圖／如上／這張／這邊／箭頭／方框／框框」，以及用來指圈圈或區塊的單獨顏色詞（藍色的、綠色的、紅色的…）。**每一處都要改寫成不看圖也懂的文字，或刪掉。** 確認整篇沒有任何「沒那張圖就看不懂」的句子，才存檔。
- 需要把某張圖／投影片「重建」給讀者時，用專用的 **`> [!FIG] 圖示重建`** 框（render.js 會渲染成靛色圖示框）把它的意思講完，框後可緊接一個 GFM 表格把結構畫出來——讓「重建的圖」成為**看得到的刻意產物**，不是藏在段落裡的「如上圖」。真的無法用文字/表格表達的（少見）才在框裡用文字描述那張圖在表達什麼；重點是**框本身就要讀得懂**。
...
- **重點框（callout）**：blockquote 開頭放標記，render.js 會渲染成彩色框——
  `> [!KEY] 標題`＝藍色重點/TL;DR、`> [!TIP]`＝綠色訣竅口訣、`> [!WARN]`＝紅色注意、`> [!QUOTE]`＝紫色金句、`> [!NOTE]`＝灰色補充、`> [!EX]`＝例子、`> [!FIG]`＝靛色圖示重建（把投影片/圖表用文字或表格重建，見 ①）。框內每行用一行、需要斷行就分行寫（會以 `<br>` 接）。**框內不要放清單或表格**（放不進去），清單/表格放在框外。
