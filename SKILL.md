...
## 前置：使用者要先開一個「帶除錯埠、已登入」的 Chrome
不要幫使用者處理密碼、不要用無頭瀏覽器登入（會觸發平台的機器人偵測）。請使用者：
1. 完全關掉現有 Chrome。
2. 用除錯埠開一個獨立設定檔的 Chrome（提供對應作業系統指令），例如 Windows：
   `chrome.exe --remote-debugging-port=9222 --user-data-dir=%USERPROFILE%\course2notes\chrome-profile`
3. 在這個視窗登入該課程平台，停在課程頁。
4. 確認 `http://localhost:9222/json/version` 有回應。
...
### 4. 轉錄（Transcribe）
`transcribe.py <audio_dir> <transcript_dir>`：**自動偵測 GPU**——有 NVIDIA GPU → 本機 faster-whisper large-v3（免費）；無 GPU → 提示使用者改用 API（`--api`，需自備 OPENAI_API_KEY 或 Deepgram），並告知每分鐘成本。
...
### 6. 產出 HTML（Render）
`render.js <notes_dir> <output.html> "<標題>"`：把 notes/ 組成一份 self-contained 漂亮 HTML（內嵌 CSS/JS、左側課程導覽、可跳轉、即時搜尋、暖色引言 callout、表格、可列印）。主色可改（品牌色）。**不綁 Notion**——這是預設交付格式，零帳號、任何瀏覽器可開。

### 7. 回報（Telemetry）
跑完呼叫 `telemetry.js`（見 README 揭露）。首次執行要顯示同意提示、可關。只回傳匿名計數，絕不含內容。
...
## 腳本清單（本技能夾內）
`recon.js`（偵察）、`sniff.js`（攔媒體）、`download.js`（yt-dlp 下載）、`transcribe.py`（whisper 本機/API）、`render.js`（產 HTML）、`telemetry.js`（匿名回報）。設定見 `config.example.json`。
