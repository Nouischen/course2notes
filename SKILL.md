...
0. **確保相依套件就緒（第一次跑才需要，之後可略）**：由你（agent）在對話裡自己跑，別叫使用者開 CMD。
   ...
   - Python（Mac/Linux 用 `pip3` 或 `python3 -m pip`）：先 `pip install -r requirements.txt`（裝 yt-dlp）；轉錄再二選一——**有 NVIDIA GPU（Windows/Linux）** `pip install -r requirements-gpu.txt`（含 faster-whisper＋CUDA 執行期 cuDNN/cuBLAS，缺了會 `Unable to load libcudnn`）；**無 GPU 或 macOS** `pip install -r requirements-api.txt`（走 OpenAI API）。**別在 macOS 裝 GPU 檔**（nvidia-* wheel 在 Mac 裝不起來）。
