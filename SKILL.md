...
對每個單元的逐字稿/文章，派子代理產出結構化筆記（可平行、不吃 GPU）。目標是**「重排版＋去口水的完整重寫」，不是逐字照搬、也不是空泛摘要**。每篇規格：

**① 自足性（最重要，逐字稿轉筆記最常犯的錯）**
筆記要能讓「**沒看過那支影片、看不到投影片**」的人完全讀懂。逐字稿裡講者「指著螢幕講」的話**絕對不能照抄進純文字筆記**——例如「左圖藍色圈」「右下角綠色圈」「看這張圖」「如上圖」「這邊這個箭頭」，讀者手上沒有那張圖，這些字等於天書。做法：
- 把那段話**背後真正的重點抽出來、改寫成能獨立站住的文字**。
  例（原逐字稿）：「左圖藍色圈是 Google 流量、右下角綠色圈是 ChatGPT，過去只有藍圈、現在多了綠圈，藍＋綠 > 原本只有藍」→
  改寫成：「講者的推論是：**AI 搜尋帶來的是額外新增的需求，不是瓜分原本的流量**。Google 流量仍是全球最大、沒有變少；ChatGPT 已擠進全球前十；兩者相加大於原本只有 Google 的時代。」
- 如果那個圖表**結構本身**才是重點（例如象限圖、比較表），就**用文字或 GFM 表格把結構重建出來**，而不是叫讀者去看不存在的圖。
- 真的非視覺不可、又無法用文字表達的（極少），就寫「（講者以圖示說明：<用文字描述那張圖在表達什麼>）」，不要留下懸空的「如上圖」。
...
**③ 版面（用 render.js 認得的語法，做出好吸收的結構）**
- 結構骨架：`# 單元標題` → 開頭一個**重點框**（見下）條列本單元 3–5 個要點 → `## 用小標分段的內容筆記` → 結尾 `## 可執行要點` 或速記小抄。
- **重點框（callout）**：blockquote 開頭放標記，render.js 會渲染成彩色框——
  `> [!KEY] 標題`＝藍色重點/TL;DR、`> [!TIP]`＝綠色訣竅口訣、`> [!WARN]`＝紅色注意、`> [!QUOTE]`＝紫色金句、`> [!NOTE]`＝灰色補充、`> [!EX]`＝例子。框內每行用一行、需要斷行就分行寫（會以 `<br>` 接）。**框內不要放清單或表格**（放不進去），清單/表格放在框外。
- **表格用 GFM pipe 格式**（`| 欄 | 欄 |` ＋ `|---|---|` 分隔列），別用 HTML `<table>`——render.js 只認 pipe 表格。比較、步驟對照、分類很適合用表格。
- **Prompt／程式碼／指令**一律用 ```` ``` ```` 圍籬區塊包起來（render.js 會渲染成深色等寬程式碼框、原樣保留），**別塞進 blockquote**——Prompt 多的課這樣才不會爛版。
- **金句/心法**用 `> [!QUOTE]` 或一般 `>` 引言；**現場 Q&A** 用 `### Q：問題` 小標＋條列回答重點（**不要用 `<details>` 摺疊**）。
- `**粗體**` 只標關鍵詞，不要整段加粗；用 `---` 分隔大段落、不濫用。
- 深度＝取代看完影片，不是大綱；但也不要為了長而灌水。

**④ 檔案**
- 大課（幾十單元）按章節分給多個子代理，各寫各的檔到 `notes/<course>/NNN_note.md`（序號檔名，`_index.md` 可放課程總覽會排在最前）。

### 6. 產出 HTML（Render）
`node render.js <notes_dir> <output.html> "<標題>" [語言]`（**`[語言]` 只支援 `zh-Hant` 或 `en`**，其他值退回 `zh-Hant`；只影響介面文字，筆記內文語言由上一步決定）：把 notes/ 組成一份 self-contained 漂亮 HTML（內嵌 CSS/JS、左側課程導覽、可跳轉、即時搜尋、**彩色重點框 callout（`> [!KEY/TIP/WARN/QUOTE/NOTE/EX]`）**、表格、可列印）。**要換品牌主色**：改 render.js CSS 裡的 `--accent`（預設 `#0091AC`）。**不綁 Notion**——這是預設交付格式，零帳號、任何瀏覽器可開。
